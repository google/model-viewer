...
import {EulerDegrees, RGB, RGBA} from '@google/model-viewer-editing-adapter/lib/main.js'
...
/**
 * Dispatch a change to the object rotation.
 */
export const dispatchObjectRotation = registerStateMutator(
    'SET_OBJECT_ROTATION', (state: State, objectRotation?: EulerDegrees) => {
      if (!objectRotation) {
        throw new Error('No valid rotation given');
      }
      if (objectRotation === state.edits.objectRotation) {
        throw new Error(
            'Do not edit objectRotation in place. You passed in the same object');
      }
      state.edits = {...state.edits, objectRotation};
    });
