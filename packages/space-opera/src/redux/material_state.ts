...
async function createMaterial(
  ...
  const mrTex = await pbr.metallicRoughnessTexture;
  const metallicRoughnessTextureId =
      mrTex ? texturesByHandle.get(mrTex)?.id : undefined;
  if (!metallicRoughnessTextureId && mrTex) {
    throw new Error(
  ...
  for (const [i, mat] of materials.entries()) {
    ...
    if (mat.normalTextureId !== oldMat.normalTextureId) {
      ...
      if (!texId) {
        ...
        await modelMaterials[i].setNormalTexture(handle);
      }
    }
    if (mat.emissiveTextureId !== oldMat.emissiveTextureId) {
      ...
      if (!texId) {
        ...
        await modelMaterials[i].setEmissiveTexture(handle);
      }
    }
