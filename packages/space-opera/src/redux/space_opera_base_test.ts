...
fdescribe('space opera base test', () => {
  ...
  fit('produces the correct materials when dispatching a GLTF action',
      async () => {
  ...
  fit('color dispatch affects edit state, but not gltf state', async () => {
    const gltf = new GltfModel(cloneJson(TEST_GLTF_JSON), null);
    dispatchGltfAndEdits(gltf);
  ...
  fit('roughness factor dispatch affects edit state, but not gltf state',
      async () => {
        const gltf = new GltfModel(cloneJson(TEST_GLTF_JSON), null);
        dispatchGltfAndEdits(gltf);
  ...
  fit('metallic factor dispatch affects edit state, but not gltf state',
      async () => {
        const gltf = new GltfModel(cloneJson(TEST_GLTF_JSON), null);
        dispatchGltfAndEdits(gltf);
  ...
  fit('material edits where roughness and metallic factors are initially undefined works',
      async () => {
  ...
  fit('getGltfEdits should produce identity edits', async () => {
    const gltf = new GltfModel(cloneJson(TEST_GLTF_JSON), null);
  ...
  fit('loading gltf without pbr works and gets proper default values',
      async () => {
  ...
  fit('loading a gltf with textures results in the correct app state',
      async () => {
  ...
  fit('applying texture edits works', async () => {
  ...
  fit('applies edits to texture doublesidedness', async () => {
    const gltf = (await createGltfWithTexture());
  ...
  fit('adding and assigning a new texture works', async () => {
    const gltf = (await createGltfWithTexture());
  ...
  fit('dispatchBaseColorTexture can be used to share textures and clear them',
      async () => {
  ...
  fit('can share or clear textures when using dispatchNormalTexture',
      async () => {
        dispatchGltfAndEdits(await createGltfWithTexture());
  ...
  fit('can share or clear textures when using dispatchEmissiveTexture',
      async () => {
        dispatchGltfAndEdits(await createGltfWithTexture());
  ...
  fit('can share or clear textures when using dispatchOcclusionTexture',
      async () => {
        dispatchGltfAndEdits(await createGltfWithTexture());
  ...
  fit('throws if we try to add duplicate mutator action names', () => {
  ...
  fit('applies camera edits correctly to a model viewer config', () => {
    const camera = {
  ...
  fit('does not add config fields if the edits do not have them set', () => {
  ...
  fit('leaves config fields alone if edits do not have them set', () => {
    const camera = {} as Camera;
    const config = {
  ...
  fit('sets correct attributes for disabled camera pitch limits', () => {
    const camera = {
  ...
  fit('sets correct attributes for enabled pitch limits', () => {
    const camera = {
  ...
  fit('correctly updates state upon dispatching camera fields', () => {
  ...
  fit('sets correct attributes for FOV limits', () => {
    const camera = {
  ...
  fit('sets the URL when calling dispatchGltfUrl', () => {
    dispatchGltfUrl('test.glb');
    expect(reduxStore.getState().gltfUrl).toEqual('test.glb');
  });

  fit('applies edits to texture emissiveFactor', async () => {
    const gltf = new GltfModel(cloneJson(TEST_GLTF_JSON), null);
    dispatchGltfAndEdits(gltf);
