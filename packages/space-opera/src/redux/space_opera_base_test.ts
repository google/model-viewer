...
import {arrayBufferEqualityTester, createBufferFromString, EulerDegrees, generatePngBlob, GlTf, GltfModel, ModelViewerConfig, RGBA} from '@google/model-viewer-editing-adapter/lib/main.js'
...
describe('space opera base test', () => {
  ...
  it('material edits where roughness and metallic factors are initially undefined works',
     async () => {
       ...
       const gltfEdits = {
         ...
         objectRotation: ([0, 0, 0] as EulerDegrees)
       };
  ...
  it('getEditedGltf after rotation dispatch should work', async () => {
    const gltf = new GltfModel(
        {
          asset: {version: '2.0'},
          nodes: [{}],
        ...
        null);
    const rotation: EulerDegrees = [12, 34, 56];

    await dispatchGltfAndEdits(gltf);
    dispatchObjectRotation(rotation);

    const newGltf = await applyEditsToStoredGltf();
    newGltf.objectRotationEulers.every(
        (x: number, i: number) => expect(x).toBeCloseTo(rotation[i]));
  });
