 ...
 * @returns `Symbol(key)`
 */
export function getOwnPropertySymbol(object: any, key: string): symbol|
    undefined {
  while (object) {
    const symbol = Object.getOwnPropertySymbols(object).find(
        (symbol) => symbol.description === key);
    if (symbol) {
      return symbol;
    }
 ...
 */
export function getOwnPropertySymbolValue<T = unknown>(
    object: any, key: string): T|undefined {
...
/**
 * Searches through hierarchy of HTMLElement until an element with a
 * non-transparent background is found
 * @param elem The element background to get
 * @returns The backgroundColor
 */
export function getBackgroundColor(elem: HTMLElement): ColorRepresentation|
    undefined {
 ...
 * @throws TypeError
 */
export function validateLiteralType<TOptions extends readonly unknown[]>(
    options: TOptions, value: typeof options[number]): void {
  if (!options.includes(value)) {
    throw new TypeError(`Validation Error: ${
        value} is not a valid value. Expected ${options.join(' | ')}`);
  }
}