class DisplayCanvas {
  ...
  SHOW_SECONDS_SHADOW(canvas, ctx, date, handLength) {
    ...
    var angle =
        ((Math.PI * 2) * ((sec / 60) + (ms / 60000))) - ((Math.PI * 2) / 4);
    ...
    // DRAW THE LENGTH.
    ctx.lineTo(
        (canvas.width / 2 + Math.cos(angle) * handLength),
  ...
  SHOW_MINUTES_SHADOW(canvas, ctx, date, handLength) {
    ...
    var sec = date.getSeconds();
    var ms = date.getMilliseconds();
    var angle = ((Math.PI * 2) * ((min / 60) + (sec / 3600) + (ms / 3600000))) -
        ((Math.PI * 2) / 4);
  ...
  update() {
    ...
    this.SHOW_MINUTES_SHADOW(
        this.canvas, this.ctx, this.initialDate, minHandLength);
    this.SHOW_HOURS_SHADOW(
        this.canvas, this.ctx, this.initialDate, hourHandLength);
