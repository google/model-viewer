...
function getExtentsFromAccessor(accessor, worldTransform, outMin, outMax)
{
    ...
    glMatrix.vec3.transformMat4(boxMin, jsToGl(accessor.min), worldTransform);

    const boxMax = glMatrix.vec3.create();
    glMatrix.vec3.transformMat4(boxMax, jsToGl(accessor.max), worldTransform);
...
class UserCamera extends gltfCamera
{
    ...
    getPosition()
    {
        ...
        const direction = glMatrix.vec3.fromValues(0, 0, this.distance);
        this.toGlobalOrientation(direction);

        const position = glMatrix.vec3.create();
        glMatrix.vec3.add(position, this.target, direction);
        return position;
    }
    ...
    lookAt(from, to)
    {
        ...
        glMatrix.vec3.subtract(difference, from, to);
        const projectedDifference = glMatrix.vec3.fromValues(from[0] - to[0], 0, from[2] - to[2]);

        this.pitch = glMatrix.vec3.angle(difference, projectedDifference);
        this.yaw = glMatrix.vec3.angle(projectedDifference, glMatrix.vec3.fromValues(1.0, 0.0, 0.0));
        this.distance = glMatrix.vec3.length(difference);
    }
    ...
    pan(x, y)
    {
        ...
        glMatrix.vec3.scale(left, left, x * this.panSpeed);

        const up = glMatrix.vec3.fromValues(0, 1, 0);
        this.toGlobalOrientation(up);
        glMatrix.vec3.scale(up, up, y * this.panSpeed);
    ...
    fitViewToScene(gltf, sceneIndex)
    {
        ...
        const direction = glMatrix.vec3.fromValues(0, 0, this.distance);
        glMatrix.vec3.add(this.getPosition(), this.target, direction);
    ...
    {
        glMatrix.vec3.rotateX(vector, vector, VecZero, -this.pitch);
        glMatrix.vec3.rotateY(vector, vector, VecZero, -this.yaw);
    }
...
class gltfBuffer extends GltfObject
{
    ...
    sefBufferFromUri(gltf, callback)
    {
        ...
        const self = this;
        axios__default['default'].get(getContainingFolder(gltf.path) + this.uri, { responseType: 'arraybuffer'})
...
class gltfMaterial extends GltfObject
{
    ...
    parseTextureInfoExtensions(textureInfo, textureKey)
    {
        ...
        if(textureInfo.extensions.KHR_texture_transform !== undefined)
        {
            ...
            glMatrix.mat3.multiply(uvMatrix, rotation, scale);
            glMatrix.mat3.multiply(uvMatrix, uvMatrix, translation);
    ...
    initGl(gltf, webGlContext)
    {
        ...
        if (this.pbrMetallicRoughness !== undefined && this.type !== "SG")
        {
            ...
            let baseColorFactor = glMatrix.vec4.fromValues(1, 1, 1, 1);
...
class gltfNode extends GltfObject
{
    ...
    getLocalTransform()
    {
        if(this.transform === undefined || this.changed)
        {
            ...
            glMatrix.mat4.fromRotationTranslationScale(this.transform, this.rotation, this.translation, this.scale);
            this.changed = false;
        }
...
async function loadGltf(file, view, additionalFiles)
{
    ...
    {
        isGlb = getIsGlb(file);
        let response = await axios__default['default'].get(file, { responseType: isGlb ? "arraybuffer" : "json" });
        json = response.data;
        data = response.data;
        filename = file;
    }
...
async function loadEnvironment(file, view, luts)
{
    ...
    if (typeof file === "string")
    {
        let response = await axios__default['default'].get(file, { responseType: "arraybuffer" });
