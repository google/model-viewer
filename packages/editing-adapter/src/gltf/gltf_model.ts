...
import {degToRad, radToDeg} from '../util/math.js';

import {DEFAULT_BASE_COLOR_FACTOR, DEFAULT_EMISSIVE_FACTOR, DEFAULT_METALLIC_FACTOR, DEFAULT_NODE_ROTATION, DEFAULT_ROUGHNESS_FACTOR, IMAGE_MIME_TYPES} from './gltf_constants.js';
import * as gltfSpec from './gltf_spec.js';
import {packGlb} from './pack_glb.js';
import {EulerDegrees, Quaternion, RGB, RGBA} from './three_dom.js';
import {unpackGlb} from './unpack_glb.js';

export {EulerDegrees, Quaternion, RGB, RGBA} from './three_dom.js';
...
export class GltfModel {
  ...
  * textureInfoProperties() {
  ...
  private get rootNode(): gltfSpec.Node|null {
    if (!this.root.nodes || this.root.nodes.length === 0) {
      return null;
    }
    // TODO: This may not be generally true, but this is temporary
    // anyway, so we'll assume it.
    return this.root.nodes[0];
  }

  get objectRotation(): Quaternion {
    if (!this.rootNode) {
      return DEFAULT_NODE_ROTATION as Quaternion;
    }
    if (this.rootNode.matrix) {
      const matrix = new THREE.Matrix4().fromArray(this.rootNode.matrix);
      const q = new THREE.Quaternion().setFromRotationMatrix(matrix);
      return [q.x, q.y, q.z, q.w];
    }

    if (this.rootNode.rotation) {
      const quat = this.rootNode.rotation as Quaternion;
      if (!quat) {
        throw new Error(`Could not convert this rotation to a quaternion: ${
            JSON.stringify(this.rootNode.rotation)}`);
      }
      return quat;
    }
  ...
  set objectRotation(rotation: Quaternion) {
    if (!this.rootNode) {
      // Just ignore
      return;
    }

    // Yes, we are clearing the *entire* transform! This is a temporary
    // solution!
    delete this.rootNode.matrix;
    this.rootNode.rotation = [...rotation];
  }

  get objectRotationEulers(): EulerDegrees {
    const q = new THREE.Quaternion().fromArray(this.objectRotation);
    const e = new THREE.Euler().setFromQuaternion(q);
    return [e.x, e.y, e.z].map(rad => radToDeg(rad)) as EulerDegrees;
  }

  set objectRotationEulers(rotation: EulerDegrees) {
    const rads = rotation.map(deg => degToRad(deg));
    const e = new THREE.Euler(rads[0], rads[1], rads[2], 'XYZ');
    const q = new THREE.Quaternion().setFromEuler(e);
    this.objectRotation = [q.x, q.y, q.z, q.w];
  }
