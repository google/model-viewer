...
import {DEFAULT_NODE_ROTATION} from './gltf_constants.js';
import {EulerDegrees, GltfModel, Material, SINGLE_PIXEL_PNG_BLOB, TextureHandle} from './gltf_model.js';
...
describe('gltf model test', () => {
  ...
  it('setting root rotation works', () => {
    const gltfJson = {
      asset: {version: '2.0'},
      nodes: [{}],
    };
    ...
    // Exercises quat-euler conversion.
    gltf.objectRotationEulers = [10, 20, 30];
    const e = gltf.objectRotationEulers;
    // No cleaner way to use toBeCloseTo for an array.
    expect(e[0]).toBeCloseTo(10);
    expect(e[1]).toBeCloseTo(20);
    expect(e[2]).toBeCloseTo(30);

    // Try identity quaternion
    gltf.objectRotation = [0, 0, 0, 1];
    gltf.objectRotationEulers.every(x => expect(x).toBeCloseTo(0));
  });
  ...
  it('properly handles root nodes with no matrix or rotation', async () => {
    const gltfJson = {
      asset: {'generator': 'FBX2glTF', 'version': '2.0'},
      nodes: [{}],
    } as GlTf;
    const model = new GltfModel(gltfJson, null);
    expect(model.objectRotation).toEqual(DEFAULT_NODE_ROTATION);

    // But we can still set it..
    const rot = [45, 0, 0] as EulerDegrees;
    model.objectRotationEulers = rot;
    expect(model.objectRotationEulers[0]).toBeCloseTo(rot[0]);
    expect(model.objectRotationEulers[1]).toBeCloseTo(rot[1]);
    expect(model.objectRotationEulers[2]).toBeCloseTo(rot[2]);
  });
